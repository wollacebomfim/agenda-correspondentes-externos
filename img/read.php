...
$query = mysqli_query($conexao, "SELECT DISTINCT * FROM agenda2 WHERE uf='$pesquisa'");
...
if($num >0){
    ...
    echo    '<tr>';
    echo    '<td>'.$row["uf"].'</td><td>'.$row["cidade"].'</td><td>'.$row["correspondente"].'</td><td>'.$row["oab"].'</td><td>'.$row["responsavel"].'</td><td><a href="info.php?id='.$row["id"].'" target="_blank"><i class="material-icons">search</td>';
    echo    '</tr>';
    }
}else{
    echo "Pesquisando...";
